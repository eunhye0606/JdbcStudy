...
--4. 이름 검색 쿼리문 구성(총점, 평균, 석차가 포함된 리스트)
SELECT *
FROM
(
    SELECT SID, NAME, KOR, ENG, MAT
...
WHERE NAME = '이시우';
-->한 줄 구성
SELECT * FROM (SELECT SID, NAME, KOR, ENG, MAT ,(KOR+ENG+MAT) AS TOT ,(KOR+ENG+MAT)/3 AS AVG ,RANK() OVER(ORDER BY(KOR+ENG+MAT) DESC) AS RANK FROM TBL_SCORE) WHERE NAME = '이시우'
;
--==>>1	이시우	90	80	70	240	80	1
